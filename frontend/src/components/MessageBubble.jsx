...
function MessageBubble({ role, content, images }) {
  ...
  return (
    <div className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
      <div
        className={`w-fit max-w-[92vw] md:max-w-[72%] px-4 py-2.5 rounded-2xl wrap-break-words overflow-hidden leading-relaxed transition-colors duration-150
        ${
          isUser
            ? "bg-[#123D2B] text-[#F2FFF7] rounded-tr-sm hover:bg-[#165238]"
            : "bg-[#10221A] text-[#E8F5EE] rounded-tl-sm"
        }`}
      >
        {images.length > 0 && (
          ...
          components={{
            ...
            a: ({ href, children }) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="text-[#20C77A] underline inline-flex items-center gap-1 hover:text-[#39FF88]"
              >
                {children}
            ...
            code: ({ className, children }) => {
              ...
              if (!className) {
                return (
                  <code className="px-1.5 py-0.5 rounded bg-white/10 text-[#E8F5EE]">
                    {value}
                  </code>
                );
              }
              ...
              return (
                <div className="my-4 overflow-hidden rounded-xl border border-white/10 bg-[#111318]">
                  <div className="flex items-center justify-between bg-[#1b1d24] border-b border-white/10 px-4 py-2">
                    ...
                    <button
                      className="flex items-center gap-1 text-xs text-[#E8F5EE] hover:text-[#39FF88]"
