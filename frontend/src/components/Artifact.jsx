      ...
      <>
        {!collapsed ? (
          <div className="flex flex-col h-full bg-[#0d0f14]">
            <div className="h-14 px-4 border-b border-white/[0.06] flex items-center gap-3 shrink-0">
              ...
              <div className="flex items-center gap-2 flex-1 min-w-0">
                <div className="flex items-center justify-center w-6 h-6 rounded-md bg-[rgba(32,199,122,0.12)] border border-[rgba(32,199,122,0.22)] shrink-0">
                  <Code2 className="text-[var(--primary)]" size={12} />
                </div>
                <div className="text-[13px] font-medium text-[var(--text-main)] truncate">
                  {artifacts[0]?.title}
                </div>
              </div>
              ...
              {canPreview && (
                <div className="flex items-center gap-1 bg-white/[0.04] border border-white/[0.06] p-1 rounded-lg">
                  ...
                  ${tab === "code" ? "bg-[var(--primary)] text-[#02140C]" : "text-[var(--text-muted)] hover:text-[var(--text-main)]"}`}
                  >
                    <Code2 size={11} /> Code
                  </button>
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-medium rounded-md transition-colors duration-150
                  ${tab === "preview" ? "bg-[var(--primary)] text-[#02140C]" : "text-[var(--text-muted)] hover:text-[var(--text-main)]"}`}
                  >
                    <Eye size={11} /> Preview
                  </button>
                </div>
              ...
              <div className="flex h-auto border-b border-white/[0.06] overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden shrink-0">
                {artifacts[0]?.files?.map((f, index) => (
                  <button
                    ...
                    className={`px-4 py-2.5 text-[11px] font-medium whitespace-nowrap transition-colors duration-150 border-r border-white/[0.05] relative cursor-pointer bg-transparent   ${activeFile === index ? "text-[var(--primary)]" : "text-[var(--text-muted)] hover:text-[var(--text-main)]"}`}
                  >
                    {f?.name}
                    {activeFile === index && (
                      <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-[var(--primary)] rounded-t-full" />
                    )}
                  </button>
                ))}
              </div>
          ...
          <div className="hidden lg:flex h-full border-l border-white/[0.06] bg-[#0d0f14] flex-col items-center py-4 gap-3 shrink-0">
            <button
              className="flex items-center justify-center w-7 h-7 rounded-lg text-[var(--primary)] hover:text-[var(--hover)] hover:bg-[rgba(32,199,122,0.08)] transition-colors duration-150 bg-transparent border-none cursor-pointer shrink-0"
            ...
            <div className="flex items-center gap-2 flex-1 min-w-0">
              <div
                className="text-[10px] font-medium text-[var(--text-muted)] tracking-widest uppercase whitespace-nowrap"
                style={{
